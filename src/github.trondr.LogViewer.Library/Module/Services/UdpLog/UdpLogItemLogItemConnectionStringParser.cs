...
namespace github.trondr.LogViewer.Library.Module.Services.UdpLog
{
    ...
    public class UdpLogItemLogItemConnectionStringParser : IUdpLogItemConnectionStringParser
    {
        ...
        private readonly Regex _connectionRegex = new Regex(@"^udp:(.+?):(\d+):(Ipv4|Ipv6)$", RegexOptions.IgnoreCase);
        ...
        public ILogItemConnection Parse(string connectionString)
        {
            ...
            if(match.Success)
            {
                ...
                var connection = _udpLogItemConnectionFactory.GetUdpLogItemConnection(connectionString, hostName, port, ipVersion);
                return connection;    
            }
            _logger.WarnFormat("Invalid connection string '{0}'. Tcp connection string must be on the format: 'udp:<hostname>:<port>:<Ipv4|Ipv6>'", connectionString);
            return null;
        }
    }
}